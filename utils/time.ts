function stringToUnixTime(inputStr: string): number {
  ...
  if (unit === "m") {
    ...
    result = now + value * 7 * stringToUnixTime("1d"); // 1 week = 604,800,000 milliseconds
  } else if (unit === "M") {
    // Adjusted for varying month lengths
    result = now + value * 30 * stringToUnixTime("1d"); // 1 month = 30 days
  } else {
