...
async function warnUser(user_id: string | number):Promise<Response> {
    ...
    if (new Boolean(data.value)) {
        ...
        return {status: 200, message: "The user has been warned. Count of warns: " + warns_count}
    }else {
        kv.set(['warns', user_id.toString()], 1); 
        return {status: 200, message: "The user has been warned. Count of warns: 1"}
    }
}
...
async function unWarnUser(user_id: string | number):Promise<Response> {
    ...
    if (new Boolean(data.value)) {
        ...
        return {status: 200, message: "The user has been unwarned. Count of warns: " + warns_count}
    }else {
